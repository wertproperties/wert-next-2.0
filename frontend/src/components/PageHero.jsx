...
export default function PageHero({ tag, title, desc, bgImage, breadcrumbs = [] }) {
  return (
    ...
    >
      ...
      <div className="absolute -bottom-1 left-0 right-0 h-16 bg-white" style={{ clipPath: 'polygon(0 100%, 100% 100%, 100% 60%, 0 0)' }} />
          ...
          <nav className="flex items-center gap-2 text-white/50 text-xs mb-5 uppercase tracking-wider">
            ...
            {breadcrumbs.map((b, i) => (
              <span key={i} className="flex items-center gap-2">
                <span>/</span>
                {b.href ? <Link to={b.href} className="hover:text-accent transition-colors">{b.label}</Link> : <span className="text-white/80">{b.label}</span>}
              </span>
            ))}
          </nav>
        )}
