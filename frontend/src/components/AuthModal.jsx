...
function SignupForm({ onSuccess }) {
  ...
  const fieldCls = 'w-full border border-slate-200 px-3 py-1.5 text-sm text-slate-800 focus:outline-none focus:border-accent transition-colors';
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <div className="relative">
            <input name="password" type={showPwd ? 'text' : 'password'} value={form.password} onChange={ch}
              required minLength={6} autoComplete="new-password" className={`${fieldCls} pr-8`} />
            <button type="button" onClick={() => setShowPwd(!showPwd)}
        ...
        <div>
          ...
          <input name="confirmPassword" type="password" value={form.confirmPassword} onChange={ch} required className={fieldCls} />
        </div>
      </div>
    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto animate-fadeIn">
        ...
        <div className="px-8 pt-8 pb-2 text-center">
          <img src={logoo} alt="WERT Logo" className="h-14 w-auto object-contain mx-auto mb-3" />
          <h1 className="font-serif text-2xl font-bold text-slate-900 mb-1">
            {tab === 'login' ? (lang === 'de' ? 'Anmelden' : 'Sign In') : (lang === 'de' ? 'Konto erstellen' : 'Create Account')}
          </h1>
        ...
        <div className="px-8">
          <div className="flex border-b border-slate-200 mb-6">
            <button
              ...
              className={`flex-1 pb-3 text-sm font-bold uppercase tracking-wider transition-colors ${tab === 'login' ? 'text-accent border-b-2 border-accent' : 'text-slate-400 hover:text-slate-600'}`}
            >
              {lang === 'de' ? 'Anmelden' : 'Sign In'}
            </button>
            <button
              onClick={() => setTab('signup')}
              className={`flex-1 pb-3 text-sm font-bold uppercase tracking-wider transition-colors ${tab === 'signup' ? 'text-accent border-b-2 border-accent' : 'text-slate-400 hover:text-slate-600'}`}
            >
