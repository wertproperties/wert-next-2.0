...
function PortalCard({ icon, title, desc, to, lang }) {
  return (
    <Link href={localePath(lang, to)} className="bg-white p-6 border border-slate-100 hover:border-accent hover:shadow-xl transition-all duration-300 group block">
        ...
        {tab === 'dashboard' && (
          <>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {cards.map(c => <PortalCard key={c.to} {...c} lang={lang} />)}
            </div>
          </>
        )}
